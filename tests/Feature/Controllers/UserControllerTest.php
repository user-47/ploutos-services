...
class UserControllerTest extends TestCase
{
    ...
    /** @test */
    public function can_view_transactions_owned()
    {
        $user1 = factory(User::class)->create();
        $user2 = factory(User::class)->create();
        (new TradeManager())->create($user1, $this->validTradeData());
        $trade = Trade::first();
        (new TradeManager())->accept($trade, $user2, 500);

        $response = $this->actingAs($user1, 'api')->getJson('api/v1/users/transactions');
        $this->assertEquals(1, $response['meta']['total']);

        $response = $this->actingAs($user2, 'api')->getJson('api/v1/users/transactions');
        $this->assertEquals(1, $response['meta']['total']);
    }
